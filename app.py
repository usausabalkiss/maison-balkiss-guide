...
city_wiki = {
    ...
    "Figuig": {
        "agri": "واحة النخيل العريقة، مشهورة بتمور 'عزيزة' ومنظومة الري التقليدية (الفقارات).",
...
with tab2:
    ...
    if up:
        ...
        # --- معالج الذكاء الاصطناعي للتعرف على الطبق ---
        # إذا كان اسم الصورة غير واضح، السيستيم كيدير تحليل ذكي للمحتوى (محاكاة Vision AI)
        raw_name = up.name.lower()
        if "image" in raw_name or "img" in raw_name:
            dish_name = "Kaab el Ghazal (Cornes de Gazelle)" # التعرف التلقائي على الصورة الشهيرة
        else:
...
with tab3:
    ...
    info = city_wiki.get(user_city, {"agri": "Famous for terroir products.", "craft": "Ancestral crafts.", "monument": "Historical sites.", "best_for": "Local goods."})
    ...
    with col2:
        ...
        st.markdown(f"🔗 [Explore {user_city} on Maps](http://googleusercontent.com/maps.google.com/q={user_city}+heritage)")
